...
export const getBlog = async (author, limit = 50) => {
  ...
  do {
    ...
    if ((result.length !== limit) || _.uniqBy(cache, 'permlink').length >= 500) {
      keepGoing = false
    }
  } while (keepGoing)

  return _.take(_.uniqBy(cache, 'permlink'), 500)
}

export const busify = (author, permlink) => `https://busy.org/@${author}/${permlink}`

export const steemify = (author, permlink) => `https://steemit.com/@${author}/${permlink}`
